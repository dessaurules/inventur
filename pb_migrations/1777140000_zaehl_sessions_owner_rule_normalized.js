...
/**
 * Create-Regel für session_owner: zusätzlich zum Roh-Body den normalisierten
 * Zielwert des Feldes prüfen (Relation → ID). Nur @request.body.* kann bei PB
 * nach der Normalisierung fehlschlagen → generisches „Failed to create record.“.
 *
 * Inhaltlich identisch zu 1777120000, nur ownerOnCreate erweitert.
 */
migrate(
  (app) => {
    ...
    const ownerOnCreate = `(session_owner = @request.auth.id || @request.body.session_owner = @request.auth.id)`
